...
```{r}
set.seed(1) # actually, I don't think this is needed

theme_set(theme_gray(base_size = 15) ) 

results <- NULL
for( itree in c(1:9, 10, 20, 50, 100, 150, 200, 250)  ){
  for(i in 1:20){
      ...
      pred.test <- predict(rf, train.precrash[-train.ix,])
      this.err <-  length(which(pred.test != train.precrash[-train.ix,]$SalePrice))/length(pred.test) 
